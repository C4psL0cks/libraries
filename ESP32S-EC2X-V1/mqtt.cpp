...
bool ECXMQTT::ConnectMQTTServer(String web, String port)
{
...
bool ECXMQTT::DisconnectMQTTServer()
{
	connected = false;
	return (tcp_mqtt.Close());
}

unsigned char ECXMQTT::Connect(String id, String user, String pass)
{
	connected = false;
...
/*unsigned char ECXMQTT::Connect(const char *id, const char *user, const char *pass) 
{
	return(Connect(id,user,pass,0,0,0,0));
}
*/
int ECXMQTT::red_data_from_3G_buffermode()
{
	int ret = 99;
...
uint8_t ECXMQTT::write_tcp(uint8_t *buf, uint8_t length)
{
	uint8_t len_let = 0;
...
String ECXMQTT::ConnectReturnCode(unsigned char input)
{
	switch (input)
	{
	case 0:
...
void ECXMQTT::Publish(char *topic, int lentopic, char *payload, int lenpay)
{
...
void ECXMQTT::Publish(String topic, String payload)
{
...
void ECXMQTT::Subscribe(char *topic, int topiclen)
{
	clear_buffer();
	buffer[0] = 0x82;
	buffer[1] = 0x00; //Remaining Length
...
void ECXMQTT::Subscribe(String topic)
{
	char chartopic[topic.length() + 2];
	unsigned char i = 0;
	for (i = 0; i < topic.length(); i++)
...
unsigned int ECXMQTT::ReadDataInBufferMode(unsigned int buf_len)
{
